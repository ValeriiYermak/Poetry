...
from info import Birthday, Email, Phone, Status


class BaseUseInterface(ABC):
    @abstractmethod
...
class ConsoleUseInterface(BaseUseInterface):
    def display_contacts(self, contacts):
        for account in contacts:
            print("_" * 50)
            print(f"Name: {account['name']}")
            print(f"Phones: {', '.join(account['phones'])}")
            print(f"Birthday: {account['birthday'].strftime('%d/%m/%Y') if account['birthday'] else ''}")
    ...
    def display_commands(self):
        ...
        print("8. Congratulate contacts with upcoming birthdays")
        print("9. Display commands")
        print("0. Exit")
...
class AddressBook(UserList):
    ...
    def display_info(self, info):
        self.ui.display_info(info)

    def get_user_input(self, prompt):
        return self.ui.get_user_input(prompt)


def main():
    console_ui = ConsoleUseInterface()
    address_book = AddressBook(console_ui)

    while True:
        address_book.display_commands()
        choice = address_book.get_user_input("Enter your choice: ")
        ...
        if choice == '1':
            ...
            address_book.display_contacts(address_book.data)
        elif choice == '6':
            # Save address book logic
            pass
        elif choice == '7':
            # Load address book logic
            pass
        elif choice == '8':
            address_book.congratulate()
        elif choice == '9':
            # Display commands logic
            pass
        elif choice == '0':
            break
        else:
            address_book.display_info("Invalid choice. Please try again.")
